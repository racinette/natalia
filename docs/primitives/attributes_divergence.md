...
| 4 | External read (non-blocking) | `handle.attributes.progress.getNowait(session)` → `{ status:"ok", value, version }` \| `{ status:"not_set" }` | Same union | Not present (`getNowait` named only in an `engine.ts` "lands in step 16" comment) | docs + REFACTOR ahead |
| 5 | Compensation-block attributes | `compensation.attributes: { undoProgress: ... }`, set via `ctx.attributes.undoProgress.set(...)` | Per-block primitive, own namespace, reads via `(await compensations.steps.<s>.find(session, …))[0]?.attributes.X.getNowait(session)` | Slot EXISTS (`StepCompensation.attributes?`, `TAttributes`), exercised in regression test 08; but the body-side `set` accessor and external `.getNowait(session)` are still placeholders (`CompensationBlockPrimitivePlane.attributes: Record<string, unknown>`) | code partial (slot only) |
...
- [ ] Whether external attribute reads use the flat `{ status, value, version }` union (per docs/REFACTOR Part 13) or another wrapper shape.
