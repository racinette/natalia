...
- **Only three ways out:** reach a normal **`return`**, be **`skip`ped** from the outside on the compensation block handle (client: look up the instance and call **`.skip(...)`** with an operator-supplied outcome when the block declares a **`result`** schema), or **halt**. An **unhandled throw inside `undo` does not fail the parent workflow**—it **halts** the compensation block instance until the underlying issue is fixed (patch/replay) or an operator intervenes.
  ...
  compensation: {
    ...
    async undo(ctx, args, info) {
      ...
      if (info.status !== "completed") {
        const forwardAttempts = await info.attempts.findMany();
