...
Step, request, queue, and topic handlers execute outside the replayed workflow body. They never use the **workflow body's** `ctx.errors`. Each primitive defines how throws and returns map to retries, dead letters, manual mode, or terminal disposition.
...
Optional **`retentionPolicy`** on request handler registration controls how long **terminal** invocation rows are kept (`resolved`, `timedOut`); it is unrelated to **`manual`** disposition on throws. See [requests](./primitives/requests.md#reference).
...
**Queues** use `QueueHandlerAttempt` rows with per-code `details` (`serialized` / `serialization_error` / `unspecified`). When `code` is set, `message` is always `string`. Query them through `handle.attempts.find(session, …)` on the client (inside `client.session`) or `ctx.attempts.find(...)` in handler callbacks.
